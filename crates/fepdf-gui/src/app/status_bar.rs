...
impl FepdfApp {
    ...
    /// The right of the bar: rotation, layout direction, view mode, fit, zoom, and the
    /// page navigation that sits between them.
    fn view_controls(&mut self, ui: &mut egui::Ui, has_doc: bool, current_page: usize) {
        // RR-15 Limit: GUI - one `if ui.button(...).clicked()` per control, in the order
        // they appear on the bar. Splitting further would separate a button from the
        // `ui.separator()` that groups it, which is what the reader is looking at.
            ...
            if has_doc {
                ...
                if ui
                    .add_sized(
                        egui::vec2(22.0, 20.0),
                        egui::Button::new(
                            egui::RichText::new(if is_r2l { "縦" } else { "横" }).size(11.0),
                        )
                        .selected(is_r2l),
                    )
                ...
                self.page_navigation(ui, current_page);
            }
        });
    }

    /// The page counter and the four buttons around it, which sit between the view
    /// controls in the right-to-left layout above.
    fn page_navigation(&mut self, ui: &mut egui::Ui, current_page: usize) {
        if ui
